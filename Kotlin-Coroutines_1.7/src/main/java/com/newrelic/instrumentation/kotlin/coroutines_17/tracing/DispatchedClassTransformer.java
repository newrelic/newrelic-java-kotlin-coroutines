...
public class DispatchedClassTransformer implements ContextClassTransformer {
	
	private Map<String, ClassMatchVisitorFactory> matchers = null;
	private final InstrumentationContextManager contextMgr;
	
	public DispatchedClassTransformer(InstrumentationContextManager mgr) {
		contextMgr = mgr;
		...
		for(Method method : match.getMethods()) {
			for(ClassAndMethodMatcher matcher : match.getClassMatches().keySet()) {
				if (matcher.getMethodMatcher().matches(MethodMatcher.UNSPECIFIED_ACCESS, method.getName(),
					...
					context.putTraceAnnotation(method, TraceDetailsBuilder.newBuilder().setTracerFactoryName(DispatchedTaskTracerFactory.TRACER_FACTORY_NAME).setDispatcher(true).setInstrumentationSourceName("CoroutinesCore").setInstrumentationType(InstrumentationType.TraceAnnotation).build());
				}
